...
import { buildFaqSchema, buildJsonLdGraph } from './seo';
import { getTagHref, injectInternalLinks } from './blogInternalLinks';
...
const CATEGORY_TAGS = {
  ...
  Comparison: ['omegle alternative', 'adult video chat', 'hot video chat'],
  Tips: ['flirty chat', 'video chat tips', 'online connections'],
...
export function enrichPost(slug, post) {
  const withLinks = injectInternalLinks(normalizeBlogContent(post.content), slug);
  const normalizedContent = formatContentForDisplay(injectHeadingIds(withLinks));
  const wordCount = countWords(normalizedContent);
  const headings = extractHeadings(normalizedContent);
  const tagLabels = post.tags || CATEGORY_TAGS[post.category] || [post.category];
  ...
  return {
    ...
    tags: tagLabels,
    tagLinks: tagLabels.map((label) => ({ label, href: getTagHref(label) })),
    faqs: post.faqs || getBlogFaqs(slug),
    seoTitle: post.seoTitle || post.title,
    image: getBlogCoverPath(slug),
    imageAbsolute: `${SITE_URL}${getBlogCoverPath(slug)}`,
    /** Prefer social PNG card for Article schema (SVG covers remain on-page) */
    socialImageAbsolute: `${SITE_URL}/blog/${slug}/opengraph-image`,
    ...
    categoryHref: `/blog/category/${String(post.category).toLowerCase()}`,
  };
}
...
export function buildBlogJsonLd(post) {
  ...
  const article = {
    ...
    image: {
      '@type': 'ImageObject',
      url: post.socialImageAbsolute || post.imageAbsolute,
      width: 1200,
      height: 630,
      caption: post.imageAlt,
    },
    thumbnailUrl: post.socialImageAbsolute || post.imageAbsolute,
    mainEntityOfPage: {
      '@type': 'WebPage',
  ...
  return buildJsonLdGraph([article, breadcrumb, buildFaqSchema(post.faqs)]);
}
